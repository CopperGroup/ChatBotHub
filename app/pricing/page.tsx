...
export default function PricingPage() {
  ...
  // Extract websiteId and currentPlanId from search parameters
  const websiteId = searchParams.get('websiteId')
  const currentPlanIdFromParams = searchParams.get('currentPlanId') // Get currentPlanId from URL query like ?currentPlanId=xyz789
  ...
  const handleSelectPlan = async (planId: string) => {
    ...
    // Ensure websiteId is available from search params
    if (!websiteId) {
        ...
        {!loading && plans.length > 0 && (
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 max-w-7xl mx-auto">
              {plans.map((plan) => {
                // Use currentPlanIdFromParams if available, otherwise fallback to user.plan?._id
                const isCurrentPlan = currentPlanIdFromParams === plan._id
