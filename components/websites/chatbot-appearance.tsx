...
const ICON_SVG_LOOKUP: { [key: string]: string } = {
  ...
  Heart: '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" height="16" width="16"><path stroke-linecap="round" stroke-linejoin="round" d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12Z" /></svg>',
  Plane: '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" height="16" width="16"><path stroke-linecap="round" stroke-linejoin="round" d="M6 12 3.269 3.125A59.769 59.769 0 0 1 21.485 12 59.768 59.768 0 0 1 3.27 20.875L5.999 12Zm0 0h7.5" /></svg>',
  Globe: '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" height="16" width="16"><path stroke-linecap="round" stroke-linejoin="round" d="M12 21a9.004 9.004 0 0 0 8.716-6.747M12 21a9.004 9.004 0 0 1-8.716-6.747M12 21c2.485 0 4.5-4.03 4.5-9S14.485 3 12 3m0 18c-2.485 0-4.5-4.03-4.5-9S9.515 3 12 3m0 0a8.997 8.997 0 0 1 7.843 4.582M12 3a8.997 8.997 0 0 0-7.843 4.582m15.686 0A11.953 11.953 0 0 1 12 10.5c-2.998 0-5.74-1.1-7.843-2.918m15.686 0A8.959 8.959 0 0 1 21 12c0 .778-.099 1.533-.284 2.253m0 0A17.919 17.919 0 0 1 12 16.5c-3.162 0-6.133-.815-8.716-2.247m0 0A9.015 9.015 0 0 1 3 12c0-1.605.42-3.113 1.157-4.418" /></svg>',
  'Arrow Right': '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" height="16" width="16"><path stroke-linecap="round" stroke-linejoin="round" d="M13.5 4.5 21 12m0 0-7.5 7.5M21 12H3" /></svg>',
  'Arrow Left': '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" height="16" width="16"><path stroke-linecap="round" stroke-linejoin="round" d="M10.5 19.5 3 12m0 0 7.5-7.5M3 12h18" /></svg>',
  'Arrow Up': '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" height="16" width="16"><path stroke-linecap="round" stroke-linejoin="round" d="M4.5 10.5 12 3m0 0 7.5 7.5M12 3v18" /></svg>',
  'Arrow Down': '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" height="16" width="16"><path stroke-linecap="round" stroke-linejoin="round" d="M19.5 13.5 12 21m0 0-7.5-7.5M12 21V3" /></svg>'
};

// This is the array of objects { name: string, svg: string } used to populate the Select
const ICON_OPTIONS_ARRAY = Object.entries(ICON_SVG_LOOKUP).map(
  ([name, svg]) => ({ name, svg })
);

// Helper function to find the name (key) of an SVG string for display in UI
const getIconNameBySvg = (
...
export function ChatbotAppearance({
  ...
  onUpdate,
  userId,
}: ChatbotAppearanceProps) {
  const [header, setHeader] = useState(
    website?.preferences?.header || "Chat Support"
  );
  const [headingText, setHeadingText] = useState(
    website?.preferences?.heading?.text ||
      "Hi there 👋 <br/> How can we help you today?"
  );
  const [headingColor, setHeadingColor] = useState(
    website?.preferences?.heading?.color || "#1f2937"
  );
  const [headingShadow, setHeadingShadow] = useState(
    website?.preferences?.heading?.shadow ?? false
  );
  const [headingShadowColor, setHeadingShadowColor] = useState(
    website?.preferences?.heading?.shadowColor || "#000000"
  );
  const [headingFontSize, setHeadingFontSize] = useState(
    website?.preferences?.heading?.fontSize || "24px"
  );

  const [gradient1, setGradient1] = useState(
    website?.preferences?.colors?.gradient1 || "#10b981"
  );
  const [gradient2, setGradient2] = useState(
    website?.preferences?.colors?.gradient2 || "#059669"
  );
  const [theme, setTheme] = useState<"light" | "dark">(
    website?.preferences?.theme || "light"
  );
  const [branding, setBranding] = useState(
    website?.preferences?.branding ?? true
  );
  const [currentLogoUrl, setCurrentLogoUrl] = useState(
    website?.preferences?.logoUrl || "./logo.png"
  );
  const [currentBgImageUrl, setCurrentBgImageUrl] = useState(
    website?.preferences?.bgImageUrl || "bg-image.png"
  );
  const [tabsMode, setTabsMode] = useState(
    website?.preferences?.tabsMode ?? true
  );
  ...
  const [showQuickActions, setShowQuickActions] = useState(
    website?.preferences?.showQuickActions ?? true
  );
  const [showHomeTabHelpSection, setShowHomeTabHelpSection] = useState(
    website?.preferences?.showHomeTabHelpSection ?? true
  );

  const [fetchedArticles, setFetchedArticles] = useState<FAQ[]>([]);
  const [selectedHomeTabHelpArticles, setSelectedHomeTabHelpArticles] =
    useState<SelectedHelpArticle[]>(
      website?.preferences?.selectedHomeTabHelpArticles || []
    );
  const [articleToAdd, setArticleToAdd] = useState<string>("");

  const [showStaffInitials, setShowStaffInitials] = useState(
    website?.preferences?.showStaffInitials ?? true
  );
  ...
  const [backgroundType, setBackgroundType] = useState<BackgroundType>(
    website?.preferences?.backgroundType || "gradient"
  );
  const [singleBackgroundColor, setSingleBackgroundColor] = useState(
    website?.preferences?.bgColor || "#FFFFFF"
  );
  ...
  const buildDeepLink = (action: QuickActionButton): string => {
    ...
    if (action.deepLinkType === "external" && action.externalUrl) {
      ...
      if (action.internalTab === "messages") {
        ...
        const itemId =
          action.internalView === "articleContent" && action.internalItemId
            ? action.internalItemId
            : "null";
        deepLink += `${view}${DEEPLINK_SEPARATOR}${itemId}`;
      } else {
  ...
  useEffect(() => {
    ...
    setLogoFileName(
      website?.preferences?.logoUrl &&
        !website.preferences.logoUrl.startsWith("data:") &&
        website.preferences.logoUrl !== "./logo.png"
        ? website.preferences.logoUrl.split("/").pop() || null
        : null
    );
    setBgImageFileName(
      website?.preferences?.bgImageUrl &&
        !website.preferences.bgImageUrl.startsWith("data:") &&
        website.preferences.bgImageUrl !== "bg-image.png"
    ...
    const fetchArticles = async () => {
      ...
      try {
        const res = await authFetch(
          `${process.env.NEXT_PUBLIC_API_BASE_URL}/websites/faqs/${website.chatbotCode}`
        );
  ...
  useEffect(() => {
    const fetchStaff = async () => {
      ...
      try {
        const res = await authFetch(
          `${process.env.NEXT_PUBLIC_API_BASE_URL}/staff/${website._id}?userId=${userId}`
        );
        if (!res.ok) {
  ...
  useEffect(() => {
      ...
      if (iframe && iframe.contentWindow) {
        ...
        const config = {
          ...
          dynamiclyAdaptToLanguage:
            website.preferences.dynamiclyAdaptToLanguage,
    ...
    const handleMessage = (event: MessageEvent) => {
      ...
      if (event.data && event.data.type === "chatbotExpand") {
        ...
        event.data &&
        (event.data.type === "chatbotCollapse" ||
  ...
  const addQuickActionButton = () => {
    setQuickActions([
      ...quickActions,
      {
        ...
        externalUrl: "",
        icon: ICON_SVG_LOOKUP.MessageSquare,
      }, // Default to SVG string directly
    ]);
  };

  const updateQuickActionButton = (
    index: number,
    field: keyof QuickActionButton,
    value: any
  ) => {
  ...
  const handleAddHelpArticle = () => {
    ...
    if (
      article &&
      !selectedHomeTabHelpArticles.some((a) => a.id === article._id)
    ) {
      setSelectedHomeTabHelpArticles([
        ...selectedHomeTabHelpArticles,
  ...
  const handleRemoveHelpArticle = (id: string) => {
    setSelectedHomeTabHelpArticles(
      selectedHomeTabHelpArticles.filter((a) => a.id !== id)
    );
  };
  ...
  const handleRemoveStaffInitial = (initial: string) => {
    setSelectedStaffInitials(
      selectedStaffInitials.filter((i) => i !== initial)
    );
  };
        ...
        <Card className="bg-white/80 backdrop-blur-sm border-0 shadow-lg rounded-3xl relative overflow-hidden h-full max-h-[calc(100vh-2rem)] overflow-y-auto">
            ...
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-slate-600 text-sm font-medium">
                  Customize your chatbot's visual style
                </p>
              </div>
            </div>
          </CardHeader>
          ...
          <CardContent className="space-y-8 relative z-10">
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-3">
                <Label className="text-slate-700 font-semibold text-sm flex items-center">
                  <HeadingIcon className="w-4 h-4 mr-2 text-purple-500" /> Chat
                  Welcome Heading
                </Label>
                <Input
                  value={headingText}
                ...
                <div className="grid grid-cols-2 gap-3 mt-3">
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setHeadingFontSize(filterFontSizeInput(e.target.value))
                      }
                ...
                <p className="text-sm text-slate-500 mt-2">
                  This text appears prominently on the chatbot's home screen.
                  HTML tags like `&lt;br/&gt;` are supported.
                </p>
              </div>
              ...
              <div className="space-y-3">
                <Label className="text-slate-700 font-semibold text-sm flex items-center">
                  <LayoutGrid className="w-4 h-4 mr-2 text-purple-500" /> Tabs
                  Mode
                </Label>
                <div className="flex items-center justify-between p-4 bg-slate-50/80 border border-slate-200/60 rounded-2xl">
                  ...
                  <Switch checked={tabsMode} onCheckedChange={setTabsMode} />
                </div>
                <p className="text-sm text-slate-500 mt-2">
                  Toggle between displaying multiple tabs (Home, Messages, Help)
                  or a single chat view.
                </p>
              </div>
            </div>
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-3">
                <Label className="text-slate-700 font-semibold text-sm flex items-center">
                  <Paintbrush className="w-4 h-4 mr-2 text-pink-500" />{" "}
                  Background Style
                </Label>
                <RadioGroup
                ...
                >
                  <Label
                    htmlFor="bg-gradient"
                    className="flex flex-col items-center justify-between rounded-md border-2 border-slate-200/60 bg-slate-50/80 p-3 hover:bg-slate-100 cursor-pointer transition-all has-[[data-state=checked]]:border-blue-500"
                  >
                    <RadioGroupItem
                      value="gradient"
                      id="bg-gradient"
                  ...
                  >
                    ...
                    <div
                      ...
                      style={{ backgroundColor: singleBackgroundColor }}
                    />
                    <span className="text-xs font-medium text-slate-900">
                      Solid Color
                    </span>
                ...
                {backgroundType === "image" && (
                  <div className="space-y-3 pt-3">
                    <Label
                      ...
                      className="text-slate-700 font-semibold text-sm flex items-center"
                    >
                      <Image className="w-4 h-4 mr-2 text-green-500" /> Upload
                      Background Image
                    </Label>
                    <input
                      id="bgImageUpload"
                    ...
                    >
                      <Upload className="w-4 h-4 mr-2" /> Select Background
                      Image
                    </Button>
                    {bgImageFileName && (
                      <p className="text-sm text-slate-600 mt-1 flex items-center">
                        Selected:{" "}
                        <span className="font-mono text-xs ml-1 bg-slate-100 p-1 rounded max-w-full truncate">
                          {bgImageFileName}
                        </span>
                      </p>
                    )}
                  ...
                  <div className="space-y-3 pt-3">
                    ...
                    <div className="flex items-center space-x-3">
                      <Input
                        ...
                        onChange={(e) =>
                          setSingleBackgroundColor(e.target.value)
                        }
              ...
              <div className="space-y-3">
                <Label
                  ...
                  className="text-slate-700 font-semibold text-sm flex items-center"
                >
                  <ImageIcon className="w-4 h-4 mr-2 text-blue-500" /> Custom
                  Logo
                </Label>
                <input
                  id="logoUpload"
            ...
            <div className="space-y-6">
                ...
                >
                  ...
                  <ChevronDown
                    className={`w-5 h-5 transition-transform duration-200 ${
                      isStaffInitialsOpen ? "rotate-180" : ""
                    }`}
                  />
                </button>
                ...
                {isStaffInitialsOpen && (
                  <div className="space-y-4 pt-4 border-l border-r border-b border-slate-200/60 rounded-b-2xl p-4 bg-slate-50/80 transition-all duration-300 ease-in-out">
                    <div className="flex items-center justify-between mb-3">
                      <span className="text-slate-900 font-medium">
                        Show Staff Initials on Home Tab
                      </span>
                      <Switch
                      ...
                      <div className="space-y-4">
                        <div className="space-y-1">
                          ...
                          <div className="flex items-center gap-2">
                              ...
                              <SelectContent>
                                ...
                                {fetchedStaffMembers.map((staff) => (
                                  <SelectItem
                                    key={staff._id}
                                    value={staff._id}
                                    disabled={selectedStaffInitials.includes(
                            ...
                            <Button
                              onClick={handleAddStaffInitial}
                              size="icon"
                              variant="outline"
                              className="h-10 w-10 border-slate-200/60 text-blue-600 hover:bg-blue-50/20"
                            >
                              <Plus className="w-4 h-4" />
                            </Button>
                          </div>
                          <p className="text-xs text-slate-500">
                            Select staff members whose initials will be shown on
                            the home tab.
                          </p>
                        </div>
                        ...
                        {selectedStaffInitials.length > 0 && (
                          <div className="space-y-2">
                            ...
                            {selectedStaffInitials.map((initial) => (
                              <div
                                key={initial}
                                className="flex items-center justify-between p-3 bg-white border border-slate-200/60 rounded-lg shadow-sm"
                              >
                                <div className="flex items-center gap-2">
                                ...
                                <Button
                                  ...
                                  size="icon"
                                  onClick={() =>
                                    handleRemoveStaffInitial(initial)
                                  }
                  ...
                  <div className="space-y-4 pt-4 border-l border-r border-b border-slate-200/60 rounded-b-2xl p-4 bg-slate-50/80 transition-all duration-300 ease-in-out">
                    ...
                    {showQuickActions && (
                      <div className="space-y-4">
                        {quickActions.map((action, index) => (
                          <Card
                            key={index}
                            className="p-4 bg-slate-50/80 border border-slate-200/60 rounded-2xl shadow-sm"
                          >
                                    ...
                                    <div>
                                      ...
                                      {action.internalView === "chat" && (
                                        <p className="text-xs text-slate-500 mt-1">
                                          Selecting "New Chat" will start a new
                                          conversation.
                                        </p>
                                      )}
                                    </div>
                                    ...
                                    <div>
                                      ...
                                      >
                                        ...
                                        <SelectContent>
                                          ...
                                          <SelectItem value="articleContent">
                                            Specific Article
                                          </SelectItem>
                                        </SelectContent>
                                      </Select>
                                      {action.internalView ===
                                        ...
                                        <div className="space-y-1">
                                          ...
                                          <p className="text-xs text-slate-500 mt-1">
                                            Select a specific article from your
                                            FAQs.
                                          </p>
                                        </div>
                                      )}
                                ...
                                >
                                  <SelectTrigger className="w-full h-10 mt-1 bg-white border-slate-200/60 rounded-lg">
                                    ...
                                    <SelectValue>
                                      <div className="flex items-center">
                                        ...
                                        {getIconNameBySvg(
                                          action.icon as string | undefined
                                        ) || "Select an icon"}
                                      </div>
                                    </SelectValue>
                                  </SelectTrigger>
                                  <SelectContent>
                                    ...
                                    {ICON_OPTIONS_ARRAY.map((iconOption) => (
                                      <SelectItem
                                        key={iconOption.name}
                                        value={iconOption.svg}
                                      >
                                        {" "}
                                        {/* SelectItem value is SVG string */}
                                        <div className="flex items-center">
                                          <span
                                            dangerouslySetInnerHTML={{
                                              __html: iconOption.svg,
                                            }}
                                            className="mr-2"
                                          />
                                          {iconOption.name}
                    ...
                    {showQuickActions && (
                      <Button
                        ...
                        variant="outline"
                        className="w-full h-12 border-slate-200/60 text-blue-600 hover:bg-blue-50/20"
                      >
              ...
              <div className="space-y-3">
                ...
                >
                  <span className="flex items-center space-x-2 font-semibold">
                    <HelpCircle className="w-4 h-4 mr-2 text-blue-500" /> Help
                    Section
                  </span>
                  <ChevronDown
                    className={`w-5 h-5 transition-transform duration-200 ${
                  ...
                  <div className="space-y-4 pt-4 border-l border-r border-b border-slate-200/60 rounded-b-2xl p-4 bg-slate-50/80 transition-all duration-300 ease-in-out">
                    ...
                    {showHomeTabHelpSection && (
                      <div className="space-y-4">
                        <div className="space-y-1">
                          <Label
                            htmlFor="add-help-article"
                            className="text-slate-700 text-sm"
                          >
                            Add Article to Help Section
                          </Label>
                          <div className="flex items-center gap-2">
                            <Select
                              value={articleToAdd}
                              ...
                              <SelectContent>
                                {fetchedArticles.length === 0 && (
                                  <SelectItem value="no-articles" disabled>
                                    No articles found. Fetching or empty.
                                  </SelectItem>
                                )}
                                {fetchedArticles.map((article) => (
                                  <SelectItem
                                    key={article._id}
                                    value={article._id}
                                    disabled={selectedHomeTabHelpArticles.some(
                                      (a) => a.id === article._id
                                    )}
                                  >
                                    {article.title}
                          ...
                          <p className="text-xs text-slate-500">
                            Select an article from your FAQs to add to the help
                            section.
                          </p>
                        </div>
                            ...
                            {selectedHomeTabHelpArticles.map((article) => (
                              ...
                              >
                                ...
                                <Button
                                  ...
                                  size="icon"
                                  onClick={() =>
                                    handleRemoveHelpArticle(article.id)
                                  }
                                  className="text-red-500 hover:bg-red-50/20"
                                >
                                  <Trash2 className="h-4 w-4" />
                        ...
                        {selectedHomeTabHelpArticles.length === 0 && (
                          <p className="text-sm text-slate-500">
                            No articles selected for the help section.
                          </p>
                        )}
                      </div>
                    )}
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-3">
                ...
                {!isProOrEnterprise && (
                  <p className="text-sm text-slate-500 mt-2">
                    Branding removal is available only on Pro and Enterprise
                    plans. Current Plan:{" "}
                    <span className="font-semibold">{website.plan.name}</span>
                  </p>
                )}
              </div>
            </div>
