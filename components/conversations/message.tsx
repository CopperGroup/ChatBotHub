...
export function Message({ message, chatName, chatEmail, isStaff, staffInfo, dashboardUserId }: MessageProps) {
  ...
  const isOutgoingMessage =
    ...
    (isStaff && isStaffMessageWithPrefix && staffInfo?.name && message.sender.split("-")[1] === staffInfo.name)
  ...
  if (isOutgoingMessage) {
    ...
    messageMetaColor = isStaffMessageWithPrefix ? "text-blue-100" : "text-emerald-100"
    ...
    if (message.sender === "bot") {
      ...
      messageBubbleClass = "bg-slate-100 text-slate-700 border border-slate-200 shadow-sm text-center"
      messageMetaColor = "text-slate-500"
  ...
  return (
    <div className={`flex ${isOutgoingMessage ? "justify-end" : "justify-start"}`}>
      <div
        className={`flex items-end space-x-2 md:space-x-3 max-w-[280px] sm:max-w-xs lg:max-w-md ${
        ...
        <div className="space-y-1 max-w-[100%]">
          ...
          <p
            className={`text-xs px-2 ${
              isOutgoingMessage ? (isStaffMessageWithPrefix ? "text-blue-300" : "text-emerald-300") : messageMetaColor
